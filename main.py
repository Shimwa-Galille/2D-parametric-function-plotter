...
def main():
    ...
    app.canvas = Canvas(app, width=1000, height=500, border=2)
...
def table_values():
    ...
    if app.coord_system == "Cartesian":
        ...
        plot_function(x_expression, y_expression, t_min, t_max, scale)
          ...
          # Calculate and draw horizontal asymptote if it exists
        ...
        # Calculate and draw oblique asymptote if it exists
...
def calculate_horizontal_asymptote(y_expression):
    x_expression = app.x_entry.get().strip()
    if y_expression=='tan(t)' or x_expression=='tan(t)':
        return None
...
def draw_horizontal_asymptote(y, scale):
    ...
    # Evaluating the table_value for the oblique asymptote
def calculate_oblique_asymptote(x_expression, y_expression):
    ...
    t_values = range(500, 10000, 500)  # Large t values to approximate infinity
    ...
    # Graphing the oblique asymptote
