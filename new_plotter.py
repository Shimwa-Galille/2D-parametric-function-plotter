...
def main():
    ...
    coord_frame = Frame(app, flow='EW', border=1,op=2, grow=False)
    ...
    # Input frame for Cartesian coordinates
    app.cartesian_frame = Frame(app, flow='EW', op=1, grow=False)

    x_frame = Frame(app.cartesian_frame, flow='EW', border=1, grow=False)
    Label(x_frame, text="x(t) = ")
    app.x_entry = Entry(x_frame)
    
    y_frame = Frame(app.cartesian_frame, flow='EW', border=1, grow=False)
    Label(y_frame, text="y(t) = ")
    app.y_entry = Entry(y_frame)
    
    t_frame = Frame(app.cartesian_frame, flow='EW', border=1, grow=True)
    ...
    # Input frame for Polar coordinates (initially hidden)
    app.polar_frame = Frame(app, flow='EW', border=1,op=2, grow=False)

    r_frame = Frame(app.polar_frame, flow='EW', border=1, grow=False)
    Label(r_frame, text="r(t) = ")
    app.r_entry = Entry(r_frame)
    
    theta_frame = Frame(app.polar_frame, flow='EW', border=1, grow=False)
    Label(theta_frame, text="θ(t) = ")
    app.theta_entry = Entry(theta_frame)
    
    pt_frame = Frame(app.polar_frame, flow='EW', border=1, grow=True)
    ...
    Button(app, text="Plot", command=table_values) # Plot_button 
    
    app.canvas = Canvas(app, width=500, height=500, border=1)
...
def show_instructions():
    instruction_win = Win(title="Instructions" ,width=100 ,height=100, grow=False)
...
def plot_polar_function(r_expression, theta_expression, t_min, t_max, scale):
    ...
    for t in range(int(t_min * 100), int(t_max * 100)):
        ...
        if prev_x is not None and prev_y is not None:
            # Check for large vertical jumps
            if abs(screen_y - prev_y) < app.height:
                app.canvas.create_line(prev_x, prev_y, screen_x, screen_y, width=2, fill=color, tag='drawing')
            else:
                prev_x, prev_y = None, None
...
def draw_tan_asymptotes(x_expression, y_expression, t_min, t_max, scale):
...
def calculate_oblique_asymptote(x_expression, y_expression):
    t_values = range(0, 10000, 500)  # Large t values to approximate infinity
...
def draw_oblique_asymptote(m, b, scale):
    ...
    # Making sure that the asymptote line is drawn fully within the canvas dimensions
    if screen_y_min > app.height:
        screen_y_min = app.height
    elif screen_y_min < 0:
        screen_y_min = 0
    
    if screen_y_max > app.height:
        screen_y_max = app.height
    elif screen_y_max < 0:
        screen_y_max = 0
    
    app.canvas.create_line(screen_x_min, screen_y_min, screen_x_max, screen_y_max, fill="red", dash=(4, 2), tag='asymptote')
